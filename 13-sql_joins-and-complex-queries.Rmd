...
```{r}
dbExecute(con,"drop table smy_film;")   
```
...
Instead of showing standard Venn diagrams showing the different JOINS, we use an analogy. For those interested thought, the typical Venn diagrams can be found [here](http://www.sql-join.com/sql-join-types/).
...
languages_ij <- language_table %>% 
  ...
  summarize(inner_joins=n()) %>% explain()
...
#### dplyr Left Outer Join

```{r left outer join}

languages_loj <- language_table %>% 
  left_join (film_table, by=c('language_id','language_id'),suffix(c(".l",".f"))) %>% 
  mutate(film_lang_id = if_else(is.na(film_id),film_id,language_id)) %>%
  group_by(language_id,name,film_lang_id) %>% 
  summarize(lojs=n()) %>%
  select (language_id,film_lang_id,name,lojs)
languages_loj
...
languages_roj <- language_table %>% 
  ...
  mutate(lang_id = if_else(is.na(name),0L,language_id)  
        ,join_type='rojs') %>% 
...
The anti join is a left outer join without the inner joined rows.  It only returns the columns from the left table.
...
```{r}
print(glue('sql_aj1 loj-null costs=',sql_aj1[1,1]));print(glue('sql_aj2 not in costs=',sql_aj2[1,1]));print(glue('sql_aj3 not exist costs=',sql_aj3[1,1]))
```
...
print(glue('sql_aj1 loj-null costs=',sql_aj1[1,1]));print(glue('sql_aj3 not exist costs=',sql_aj3[1,1]))
```
...
This example belongs in a separate chapter on table manipulation, (union, intersection, except/minus)
...
### which store has the largest revenue stream?
```{r}
rs <- dbGetQuery(con,
...
```{r}
rs <- dbGetQuery(con,
"SELECT r.store_id store,sum(CASE WHEN payment_id IS NULL THEN 1 ELSE 0 END) open
...
#### Dplyr Modify the following dplyr code to match the SQL output from above.
...
est_bal <-rental_table %>% 
    ...
    mutate(missing = ifelse(is.na(payment_id),1,0)
          ,found   = ifelse(!is.na(payment_id),1,0)
          ) %>%
    summarize(open=sum(missing,na.rm = TRUE)
...
act_bal <-rental_table %>% 
    ...
    inner_join(film_table, by=c('film_id','film_id'),suffix = c(".i", ".f")) %>% head
...
dplyr_tbl_loj <-
    ...
    filter(is.na(rental_id)) %>% select(c('first_name','last_name','email')) 
...
# View the graph
render_graph(graph,height=1000)
```
