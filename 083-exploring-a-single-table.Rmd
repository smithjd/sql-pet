...
On an annual basis, are sales dollars and orders trending up, down or flat? We begin with annual revenue and number of orders.  
...
```{r average dollars per sale - v2, fig.height=4, fig.width=4, fig.cap = "Total Number of orders by year"}
...
```{r average dollars per sale - , fig.height=4, fig.width=4, fig.cap = "Yearly Average Dollars per Sale"}
...
Here we can see that Adventureworks has two larger swings in month-over-month variation. Once in 2011-2012 and also in 2014. We now know that 2013 was the highest producing and most stable sales year so far. However, we are seeing new patterns in 2014. 

### Comparing dollars and orders to 2013

To look at dollars and the number of orders together, we compare the monthly data to the yearly average for 2012 since it's a full year. 
...
Using 2013 as a baseline, we'll see how 2014 compaires against the annual avg_dollars for 2013.

```{r}
start_year
```

Express monthly data relative to 2013 average:
The numerator in this case is the 2014 total_soh_dollars multiplied by 100. 

```{r , fig.cap = "Miscellaneous plots"}
monthly_sales_base_year_relative_to_2013 <- monthly_sales %>%
  mutate(
    diff = (100 * total_soh_dollars) / start_year$avg_dollars,
    number_of_orders = (100 * soh_count) / start_year$avg_count
  ) %>%
  filter(year(orderdate) %in% c(2013,2014)) %>%
  ungroup()

monthly_sales_base_year_relative_to_2013 <- monthly_sales_base_year_relative_to_2013 %>%
  select(orderdate, diff, `# of orders` = number_of_orders) %>%
  pivot_longer(-orderdate,
    names_to = "relative_to_2013_average",
    values_to = "amount"
  )

monthly_sales_base_year_relative_to_2013 %>%
  ggplot(aes(orderdate, amount, color = relative_to_2013_average)) +
  geom_line() +
...
```{r , warning=FALSE, fig.cap = "Percent change - Channel Comparison"}
...
```{r , warning=FALSE, fig.cap = "Number of Sales Orders by Channel"}
...
```{r , warning=FALSE, message=FALSE, fig.cap = "Lagged Sales by Channel"}
