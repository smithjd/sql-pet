...
ggplot(
  ...
  labs(
    ...
    x = paste0("Monthly - between ", min_soh_dt, " - ", max_soh_dt),
    y = "Number of Sales Recorded",
     caption = "green is corrected, orange is as recorded in the database"
  )
```
...
x <- bind_rows(mon_sales,monthly_sales_rep_as_is, .id = "date_kind")
```
...
ggplot(
  data = x,
  aes(x = orderdate, y = soh_count, color = date_kind, fill=date_kind)
) +
