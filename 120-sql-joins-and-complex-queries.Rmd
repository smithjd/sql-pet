...
The data in the DVD rental system is too clean to show some of the issues one comes across in the real world.  In the following code blocks, we look at one row in the film table where the film_id = 1.  We first reinitialize language_id to 1 and display the row.  
...
The output tells us that there are `r rs$n` inner joins occurred between the language_table and the film_table.
...
The lojs column returns the number of rows found on the keys from the left table, language, and the right table, the film table.  For the "English" row, the language_id and film_lang_id match and `r rs$lojs[1]` inner joins were performed.  For all the other languages, there was only 1 join and they all came from the left outer table, the language table, language_id's 2 - 6.  The right table, the film table returned NA, because no match was found.
...
The rojs column returns the number of rows found on the keys from the right table, film, and the left table, language.  For the "English" row, the language_id and film_lang_id match and a 999 inner joins were performed.  For language_id = 10  from the right table, there were 2 joins to a non-existant row in the language table on the left.  
...
3.  Language_id = NULL is the right outer join.
...
How are the stores performing?  The SQL code shows the rental payments made to each store in the business.
...
# film <- dplyr::tbl(con, "film") # Lazy
# x<-film %>% filter(film_id == 1001)  %>% explain()
#
