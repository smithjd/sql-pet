...
The following code demonstrates the `n` argument to `dbFetch` to specify the maximum number of rows to return. Normally, you would probably pick some fixed number of records to return each time, but this code shows that you can vary the number of records returned by each call to `dbFetch`.
```{r}
