...
```{r}
SalesRepOrders_Fiscal_Year_with_Imputed_values <- dbGetQuery(con,
"
with wo_adj_so_dates as
       (SELECT 'orig_date' date_type
...
sp_print_df(SalesRepOrders_Fiscal_Year_with_Imputed_values)
```


```{r}
for (fy_yr in 2011:2014){
  fyr_data <- SalesRepOrders_Fiscal_Year_with_Imputed_values %>% filter(yyyy == fy_yr)
  print(fyr_plot(fyr_data))
}
```
...
"with adj_so_dates as
       (SELECT 'adj_date' date_type,salespersonid
             , min(date_part('year'::text, so_adj_date(soh.orderdate, onlineorderflag) + '6 mons'::interval))  AS fiscalyear
             , min(TO_CHAR(so_adj_date(soh.orderdate, onlineorderflag), 'YYYY')) yyyy
             , TO_CHAR(so_adj_date(soh.orderdate, onlineorderflag), 'YYmm') yymm
             , COUNT(*) * 1.0 so_cnt 
          ...
          FROM sales.salesorderheader soh
         where not onlineorderflag
        group by TO_CHAR(so_adj_date(soh.orderdate, onlineorderflag), 'YYmm'),salespersonid
        )
select * from adj_so_dates order by salespersonid,yymm desc")
