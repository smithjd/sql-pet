...
ggplot(data=monthly_sales
       ,aes(x=mo,y=so_dollars,color=as.factor(yr),
            group = as.factor(yr))) + # Removes msg: Each group consists of only one observation. Do you need to adjust the group aesthetic?
...
monthly_sales_d <- tbl(con, in_schema("sales", "salesorderheader")) %>% 
  ...
  group_by(order_date) %>% 
  summarize(min_orderdate = min(orderdate), 
