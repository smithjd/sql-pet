...
```
##                                                          QUERY PLAN
## 1                   Sort  (cost=1166.16..1167.66 rows=599 width=57)
## 2                               Sort Key: (sum(f.rental_rate)) DESC
## 3      ->  HashAggregate  (cost=1131.04..1138.53 rows=599 width=57)
## 4                                          Group Key: c.customer_id
## 5         ->  Hash Join  (cost=1009.99..1120.15 rows=1452 width=23)
