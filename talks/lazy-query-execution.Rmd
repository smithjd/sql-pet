...
## The same information is available with R code in your console:
...
* There are some functions like `date` are ambiguous, so are passed to SQL as SQL code if before `collect` but are executed by R if they appear afterward.
