...
1. Enter the following command prompt: `pg_restore -d dvdrental -U postgres dvdrental.tar` 

Alternatively, if you did not unzip the file to `.tar` format, you can use psql to restore the database from the `.sql` file. Enter the following command at the **psql command line**: `psql -U postgres -d dvdrental -f restore.sql`. Note that this method is not recommended for the `dvdrental` database because it was backed up with the intent to be restored only from `.tar` format. Restoring via this method does work, but errors are introduced that do not occur when restoring the `.tar` file. 
